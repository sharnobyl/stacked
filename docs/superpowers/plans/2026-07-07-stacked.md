...
- [x] **Step 3: Write `LICENSE`** — standard MIT license text, year 2026, copyright holder "Stacked contributors".

- [x] **Step 4: Minimal sources so targets compile**
...
- [x] **Step 1: Write the failing tests** — `Tests/StackedCoreTests/StackItemTests.swift`:
...
- [x] **Step 2: Run to verify failure** — `swift test` → FAIL (StackItem not defined).

- [x] **Step 3: Implement** — `Sources/StackedCore/StackItem.swift`:
...
- [x] **Step 4: Run tests** — `swift test` → all PASS.
...
- [x] **Step 5: Commit** — `git add -A && git commit -m "feat: StackItem model with previews and kinds"`
...
- [x] **Step 1: Write the failing tests** — `Tests/StackedCoreTests/StackStoreTests.swift`:
...
- [x] **Step 2: Run to verify failure** — `swift test` → FAIL (StackStore not defined).

- [x] **Step 3: Implement** — `Sources/StackedCore/StackStore.swift`:
...
- [x] **Step 5: Commit** — `git add -A && git commit -m "feat: StackStore with FIFO/LIFO direction, pop, remove, clear"`
...
- [x] **Step 1: Write the failing tests** — `Tests/StackedCoreTests/PasteboardIOTests.swift`:
...
- [x] **Step 2: Run to verify failure** — `swift test` → FAIL (PasteboardIO not defined).

- [x] **Step 3: Implement** — `Sources/StackedCore/PasteboardIO.swift`:
...
- [x] **Step 5: Commit** — `git add -A && git commit -m "feat: lossless pasteboard snapshot/write with self-write marker and size cap"`
...
- [x] **Step 1: Write the failing tests** — `Tests/StackedCoreTests/ClipboardMonitorTests.swift`:
...
- [x] **Step 2: Run to verify failure** — `swift test` → FAIL (ClipboardMonitor not defined).

- [x] **Step 3: Implement** — `Sources/StackedCore/ClipboardMonitor.swift`:
...
- [x] **Step 5: Commit** — `git add -A && git commit -m "feat: clipboard monitor with self-write suppression"`
...
- [x] **Step 1: Implement** — `Sources/StackedApp/HotkeyManager.swift`:
...
- [x] **Step 2: Temporary manual-check wiring** — replace `Sources/StackedApp/main.swift`:
...
> Executor note: verified compile + launch; interactive keypress check deferred to Task 11 QA (autonomous session).
...
- [x] **Step 4: Commit** — `git add -A && git commit -m "feat: global Shift-Cmd-C hotkey via Carbon"`
...
- [x] **Step 1: Implement** — `Sources/StackedApp/PasteInterceptor.swift`:
...
- [x] **Step 2: Verify it compiles** — `swift build` → `Build complete!`

- [x] **Step 3: Commit** — `git add -A && git commit -m "feat: Cmd-V event tap that swaps in the next stack item"`
...
- [x] **Step 1: Implement AppState** — `Sources/StackedApp/AppState.swift`:
...
- [x] **Step 2: Implement StackView** — `Sources/StackedApp/StackView.swift`:
...
- [x] **Step 3: Verify it compiles** — `swift build` → `Build complete!`

- [x] **Step 4: Commit** — `git add -A && git commit -m "feat: SwiftUI stack panel content with banner, list, empty state"`
...
- [x] **Step 1: Implement StackPanel** — `Sources/StackedApp/StackPanel.swift`:
...
- [x] **Step 2: Implement StatusItemController** — `Sources/StackedApp/StatusItemController.swift`:
...
- [x] **Step 4: Commit** — `git add -A && git commit -m "feat: floating stack panel and menu-bar status item"`
...
- [x] **Step 1: Implement** — `Sources/StackedApp/AppController.swift`:
...
- [x] **Step 2: Replace `Sources/StackedApp/main.swift`**
...
- [x] **Step 3: Build and smoke-test** — `swift build` → `Build complete!`. If a GUI session is available, run `.build/debug/StackedApp`, press Shift-Cmd-C, verify the panel appears, copy some text in another app, verify it appears in the panel. Full QA happens in Task 11.
...
- [x] **Step 4: Run all tests** — `swift test` → all PASS.

- [x] **Step 5: Commit** — `git add -A && git commit -m "feat: wire app controller, panel, hotkey, monitor, interceptor"`
...
- [x] **Step 1: Write `Resources/Info.plist`**
...
- [x] **Step 2: Write `Scripts/bundle.sh`** (then `chmod +x Scripts/bundle.sh`)
...
- [ ] **Step 5: Run the QA checklist manually if a GUI session is available.** _(DEFERRED: needs human hands; bundle verified to launch/quit. See LOOP.md Blockers.)_ Record any failures as bugs and fix before proceeding (use superpowers:systematic-debugging). If headless, note that QA is pending a GUI session.

- [x] **Step 6: Commit** — `git add -A && git commit -m "feat: app bundling script, Info.plist, manual QA checklist"`
...
- [x] **Step 2: Write `.github/workflows/release.yml`**
...
- [x] **Step 3: Commit** — `git add -A && git commit -m "ci: build/test workflow and tag-triggered release"`
...
- [x] **Step 1: Write `README.md`** — must contain, in this order:
...
- [x] **Step 2: Write `CONTRIBUTING.md`** — short: dev setup (`swift build`, `swift test`, `Scripts/bundle.sh`), run manual QA (`docs/QA.md`) for changes touching the event tap/panel, PRs welcome, code style follows existing files, no new dependencies without discussion.
...
- [x] **Step 4: Commit** — `git add -A && git commit -m "docs: README, CONTRIBUTING, CHANGELOG"`
...
- [x] **Step 3: Confirm repo contents render** — `gh repo view --web` optional; verify README, license detection, and Actions tab via `gh api repos/{owner}/{repo} --jq .license.spdx_id` → `MIT`.

- [ ] **Step 4: Tag v0.1.0 only after manual QA has passed** _(DEFERRED per hard rule: waiting on human GUI QA — see docs/QA.md)_ (Task 11 Step 5). If QA is still pending a GUI session, stop here and report; do NOT tag a release that has never been run.
